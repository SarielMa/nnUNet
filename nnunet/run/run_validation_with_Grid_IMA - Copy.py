...
if __name__ == "__main__":
    ...
    #
    folderDict = ["IMAD2Grid","IMAD4Grid","IMAD5Grid" ]
    ...
    if choice==0:
        for epsMax in [15,20,25,30,35]:
            for delta in [3,4,5,6,7]:  
                ...
                model = mn1+str(task)+mn2+str(epsMax)+mn3+str(delta)+mn4
                name = mn1+str(task)+mn2+str(epsMax)+mn3+str(delta)
                get_avgDice(name, join(basePath, model), noises, task)
                
                
                
    if choice==1:
        for epsMax in [9,12,15,18,21]:
